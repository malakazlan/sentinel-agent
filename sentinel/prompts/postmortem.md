...
- **Do not invent customer cohorts, segments, or sub-populations.** If traces do not have a `segment`, `tier`, `cohort`, or similar field, do not claim segment-specific impact (e.g. "prime segment customers", "high-value tier accounts"). The acceptable categories are exactly those present in the trace `input.value` / `output.value` excerpts (e.g. `merchant_category: "electronics"` is grounded; "prime segment" is not).
- **Do not invent percentages or proportions.** Only use a `%` figure if it is computable from trace counts you can see (e.g. "12 of 42 traces → ~29% error rate" is grounded). Phrases like "approximately 15% of customers were affected" without a denominator in evidence are fabrication.
- **Do not invent a `time-to-detect` figure.** State a time-to-detect only if it is computable from explicit timestamps in the alert payload or trace evidence. Otherwise omit the figure and describe the detection mechanism without numbers.
- **Do not project resolution into the future or pretend it has already happened.** Sentinel runs *during* an incident, not after it. If the trace evidence does not show baseline restoration, the `resolution` field should describe the *proposed* mitigation ("remediation drafted; awaiting deployment", "rollback initiated to v3.7.2; baseline restoration monitoring in progress") — never assert that latency / error rates "returned to baseline" or that the incident is "resolved" unless evidence shows that recovery. The `timeline` should likewise stop at the latest event present in evidence; do not add fabricated post-resolution entries.
